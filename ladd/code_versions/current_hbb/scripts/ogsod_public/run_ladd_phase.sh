...

{
  ...
  echo "fgd_alpha=${FGD_ALPHA:-0.0001}"
  echo "fgd_beta=${FGD_BETA:-0.00005}"
  echo "fgd_gamma=${FGD_GAMMA:-0.001}"
  echo "fgd_lambda=${FGD_LAMBDA:-${FGD_RELATION_WEIGHT:-0.0}}"
  echo "fgd_normalization_mode=${FGD_NORMALIZATION_MODE:-original}"
  ...
  echo "ld_vlr_weight=${LD_VLR_WEIGHT:-0.25}"
  echo "ld_main_weight=${LD_MAIN_WEIGHT:-0.25}"
  echo "ld_allow_empty_vlr=${LD_ALLOW_EMPTY_VLR:-1}"
  echo "cmdistill_feature_weight=${CMDISTILL_FEATURE_WEIGHT:-1.0}"
  echo "cmdistill_relation_weight=${CMDISTILL_RELATION_WEIGHT:-1.0}"
  echo "cmdistill_logit_weight=${CMDISTILL_LOGIT_WEIGHT:-1.0}"
  echo "cmdistill_temperature=${CMDISTILL_TEMPERATURE:-4.0}"
  echo "cmdistill_max_tokens=${CMDISTILL_MAX_TOKENS:-512}"
  echo "cmdistill_min_confidence=${CMDISTILL_MIN_CONFIDENCE:-0.05}"
  ...
  echo "cclkd_temperature_min=${CCLKD_TEMPERATURE_MIN:-0.5}"
  echo "cclkd_temperature_max=${CCLKD_TEMPERATURE_MAX:-5.0}"
  echo "cclkd_entropy_scale=${CCLKD_ENTROPY_SCALE:-5.0}"
} > "$MANIFEST"
...
if [[ "$TASK" == "hbb" ]]; then
  cmd+=(
	    ...
	    --fgd-alpha "${FGD_ALPHA:-0.0001}"
	    --fgd-beta "${FGD_BETA:-0.00005}"
	    --fgd-gamma "${FGD_GAMMA:-0.001}"
	    --fgd-lambda "${FGD_LAMBDA:-${FGD_RELATION_WEIGHT:-0.0}}"
	    --fgd-normalization-mode "${FGD_NORMALIZATION_MODE:-original}"
	    ...
	    --ld-vlr-weight "${LD_VLR_WEIGHT:-0.25}"
	    --ld-main-weight "${LD_MAIN_WEIGHT:-0.25}"
	    --ld-allow-empty-vlr "${LD_ALLOW_EMPTY_VLR:-1}"
	    --cmdistill-feature-weight "${CMDISTILL_FEATURE_WEIGHT:-1.0}"
	    --cmdistill-relation-weight "${CMDISTILL_RELATION_WEIGHT:-1.0}"
	    --cmdistill-logit-weight "${CMDISTILL_LOGIT_WEIGHT:-1.0}"
	    --cmdistill-temperature "${CMDISTILL_TEMPERATURE:-4.0}"
	    --cmdistill-max-tokens "${CMDISTILL_MAX_TOKENS:-512}"
	    --cmdistill-min-confidence "${CMDISTILL_MIN_CONFIDENCE:-0.05}"
    ...
    --cclkd-temperature-min "${CCLKD_TEMPERATURE_MIN:-0.5}"
    --cclkd-temperature-max "${CCLKD_TEMPERATURE_MAX:-5.0}"
    --cclkd-entropy-scale "${CCLKD_ENTROPY_SCALE:-5.0}"
  )
