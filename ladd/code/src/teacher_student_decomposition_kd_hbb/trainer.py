...
class TeacherStudentDecompositionKDNRRLTeacherUAuxTrainer(DetectionTrainer):
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides: dict | None = None, _callbacks: dict | None = None):
        ...
        self.dkd_cfg = {
            ...
            "mgd_mask_ratio": float(overrides.pop("mgd_mask_ratio", 0.5)),
            "crosskd_temperature": float(overrides.pop("crosskd_temperature", 2.0)),
            "crosskd_pred_weight": float(overrides.pop("crosskd_pred_weight", 1.0)),
            "crosskd_feat_weight": float(overrides.pop("crosskd_feat_weight", 0.25)),
            "crosskd_teacher_conf_threshold": float(overrides.pop("crosskd_teacher_conf_threshold", 0.25)),
            "c2kd_selection_threshold": float(overrides.pop("c2kd_selection_threshold", 0.25)),
    ...
    def _make_criterion(self, model, teacher_model):
        return TeacherStudentDecompositionKDNRRLTeacherUAuxLossHBB(
            ...
            mgd_mask_ratio=self.dkd_cfg["mgd_mask_ratio"],
            crosskd_temperature=self.dkd_cfg["crosskd_temperature"],
            crosskd_pred_weight=self.dkd_cfg["crosskd_pred_weight"],
            crosskd_feat_weight=self.dkd_cfg["crosskd_feat_weight"],
            crosskd_teacher_conf_threshold=self.dkd_cfg["crosskd_teacher_conf_threshold"],
            c2kd_selection_threshold=self.dkd_cfg["c2kd_selection_threshold"],
