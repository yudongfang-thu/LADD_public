...
- batch 沿用容量表：`n/s/m=64`, `l=32`, `x=16`。
...
warmup_bias_lr = 0.1
deterministic = true
batch = n/s/m:64, l:32, x:16
```

该 batch 表只描述 OGSOD mosaic100 主表协议。no-mosaic fallback / robustness 结果沿用历史 no-mosaic baseline 设置，其中 YOLO11m 是 batch32。
