...
control_ready() {
  ...
  if [[ -z "$DET_CSV" || -z "$RAW_CSV" ]]; then
    echo "[$(date '+%F %T')] waiting for low-lr result files det=${DET_CSV:-missing} raw=${RAW_CSV:-missing}" >&2
    return 1
  fi
  python3 - "$DET_CSV" "$RAW_CSV" "$MIN_ROWS" <<'PY'
import csv
import sys
...
def read(path):
    with Path(path).open(newline="", errors="replace") as f:
        return [{(k.strip() if k else k): v for k, v in r.items()} for r in csv.DictReader(f)]
...
def summary(path):
    ...
    return rows, fv(best, key), fv(best, key50), fv(last, key)

det, det_best, det_best_ap50, det_last = summary(sys.argv[1])
raw, raw_best, raw_best_ap50, raw_last = summary(sys.argv[2])
min_rows = int(sys.argv[3])
print(
    f"det_rows={len(det)} det_best={det_best:.5f} det_last={det_last:.5f} "
    f"raw_rows={len(raw)} raw_best={raw_best:.5f} raw_last={raw_last:.5f}"
)
if len(det) < min_rows or len(raw) < min_rows:
    sys.exit(1)
if det_best >= 0.358 and det_last >= 0.340:
    sys.exit(0)
sys.exit(1)
PY
}
...
launch_fused_shared() {
  ...
  run_tag="reachable_fused_${suffix}_lowlr1e3_nowarmup_ir2rgb_yolo11n_e200_b64_img512_s0_${stamp}"
  project_dir="runs_public/dronevehicle_method_search/sub2k_seed0_fullval/reachable_fused_shared/${suffix}"
  log_root="logs/dronevehicle_method_search/sub2k_seed0_fullval/reachable_fused_shared/${suffix}"
  ...
  echo "[$(date '+%F %T')] launching reachable fused shared mode=${mode} gpu=${gpu} run=${run_tag}"
  nohup env \
    PYTHON_BIN=/root/shared-nvme/venvs/ladd312/bin/python \
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    MODEL="$STUDENT" SAR_BASELINE="$STUDENT" RGB_TEACHER="$TEACHER" \
    INIT_TYPE="reachable_fused_shared_${suffix}_dronevehicle" \
    PAPER_RUN=0 PAPER_PROTOCOL_ID="dronevehicle_sub2k_lowlr_nowarmup_reachable_fused_shared_${suffix}" \
    ...
    B_DETECTOR_SOURCE="$STUDENT" B_DECOMP_SOURCE="$DECOMP" B_LOAD_STUDENT_SPLIT=1 B_LOAD_STUDENT_REACHABILITY=0 \
    ...
    ALPHA_KD=1.0 LAMBDA_REACH=0.0 LAMBDA_REC=0.0 LAMBDA_TASKL=0.0 ALPHA_S_REC=0.05 KD_CALIBRATION_MODE=affine \
    FUSED_SHARED_MODE="$mode" FUSED_SHARED_ALIGN_WEIGHT=0.25 FUSED_SHARED_REACH_WEIGHT=0.50 FUSED_SHARED_KD_WEIGHT=1.0 FUSED_SHARED_TASK_WEIGHT=0.25 \
    ...
    > "$outer_log" 2>&1 &
  echo "$!" > "$pid_path"
  echo "[$(date '+%F %T')] reachable fused pid=$(cat "$pid_path") outer=${outer_log}"
}

launch_nofusion_splitrec_control() {
  local gpu="$1"
  local stamp run_tag project_dir log_root phase_log_dir outer_log pid_path run_name
  stamp=$(date +%Y%m%d_%H%M%S)
  run_tag="reachable_fused_c0_nofusion_splitrec_lowlr1e3_nowarmup_rgb_yolo11n_e200_b64_img512_s0_${stamp}"
  project_dir="runs_public/dronevehicle_method_search/sub2k_seed0_fullval/reachable_fused_shared/c0_nofusion_splitrec"
  log_root="logs/dronevehicle_method_search/sub2k_seed0_fullval/reachable_fused_shared/c0_nofusion_splitrec"
  phase_log_dir="${log_root}/${run_tag}_gpu${gpu}"
  outer_log="${log_root}/${run_tag}_gpu${gpu}.outer.log"
  pid_path="${log_root}/${run_tag}_gpu${gpu}.pid"
  run_name="${run_tag}_b"
  mkdir -p "$project_dir" "$phase_log_dir" "$log_root"
  echo "[$(date '+%F %T')] launching no-fusion split-rec control gpu=${gpu} run=${run_tag}"
  nohup env \
    PYTHON_BIN=/root/shared-nvme/venvs/ladd312/bin/python \
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    MODEL="$STUDENT" SAR_BASELINE="$STUDENT" RGB_TEACHER="$TEACHER" \
    INIT_TYPE=reachable_fused_shared_c0_nofusion_splitrec_dronevehicle \
    PAPER_RUN=0 PAPER_PROTOCOL_ID=dronevehicle_sub2k_lowlr_nowarmup_reachable_fused_shared_c0_nofusion_splitrec \
    DISABLE_OGSOD_PROTOCOL_GUARD=1 \
    DATA_CFG="$RGB_YAML" TEACHER_DATA_CFG="$IR_YAML" \
    GPU_ID="$gpu" SEED=0 BATCH_SIZE=64 STRICT_BATCH_SIZE=1 WORKERS=8 IMGSZ=512 \
    EPOCHS=200 PATIENCE=200 PHASE_MIN_EPOCHS=200 \
    ...
    STUDENT_BRANCH_MODE=split STUDENT_DETECT_MODE=raw TEACHER_FEATURE_MODE=decomposed UNLEARNABLE_HIDDEN_RATIO=1.0 \
    USE_MASK=0 USE_FG_MASK_FOR_REACH=1 USE_FG_MASK_FOR_REC=0 TASK_LOSS_FG_ONLY=1 \
    ALPHA_KD=0.0 LAMBDA_REACH=0.0 LAMBDA_REC=0.0 LAMBDA_TASKL=0.0 ALPHA_S_REC=0.05 KD_CALIBRATION_MODE=affine \
    FUSED_SHARED_MODE=none FUSED_SHARED_ALIGN_WEIGHT=0.0 FUSED_SHARED_REACH_WEIGHT=0.0 FUSED_SHARED_KD_WEIGHT=0.0 FUSED_SHARED_TASK_WEIGHT=0.0 \
    COS_LR=1 LR0=0.001 LRF=0.1 MOMENTUM=0.937 WEIGHT_DECAY=0.0005 OPTIMIZER=SGD \
    WARMUP_EPOCHS=0.0 WARMUP_BIAS_LR=0.0 WARMUP_MOMENTUM=0.937 \
    MOSAIC=0.0 CLOSE_MOSAIC=0 CLOSE_AT_EPOCH= MIXUP=0.1 CUTMIX=0.0 DEGREES=0.0 PERSPECTIVE=0.0 TRANSLATE=0.1 SCALE=0.5 FLIPLR=0.5 FLIPUD=0.0 HSV_H=0.0 HSV_S=0.0 HSV_V=0.0 ERASING=0.0 \
    SAVE_PERIOD=25 EXIST_OK=0 \
    ...
    > "$outer_log" 2>&1 &
  echo "$!" > "$pid_path"
  echo "[$(date '+%F %T')] nofusion split-rec pid=$(cat "$pid_path") outer=${outer_log}"
}
...
echo "[$(date '+%F %T')] reachable fused shared queue started threshold=${THRESHOLD_MB} min_rows=${MIN_ROWS}"
while ! control_ready; do
  sleep "$POLL_SECONDS"
done

gpu=$(wait_for_gpu)
launch_nofusion_splitrec_control "$gpu"
sleep 180

gpu=$(wait_for_gpu)
launch_fused_shared "$gpu" sum "sum_mlp_cap2"
sleep 180

gpu=$(wait_for_gpu)
launch_fused_shared "$gpu" concat "concat_mlp_cap2"

echo "[$(date '+%F %T')] reachable fused shared queue finished launching"
