...
REFINE_MIN_ROWS="${REFINE_MIN_ROWS:-200}"
...
refine_ready() {
  local weak_csv decay_csv
  weak_csv=$(latest_csv 'runs_public/dronevehicle_method_search/sub2k_seed0_fullval/dsn_shared_private/s2_refine_variants/w0p25_nodecay/*/results.csv')
  decay_csv=$(latest_csv 'runs_public/dronevehicle_method_search/sub2k_seed0_fullval/dsn_shared_private/s2_refine_variants/w1p0_decay60_160_final0/*/results.csv')
  if [[ -z "$weak_csv" || -z "$decay_csv" ]]; then
    echo "[$(date '+%F %T')] waiting for refine csv files weak=${weak_csv:-missing} decay=${decay_csv:-missing}" >&2
    return 1
  fi
  python3 - "$REFINE_MIN_ROWS" "$weak_csv" "$decay_csv" <<'PY'
import csv
import sys
...
launch_shuffle() {
  ...
  run_tag="dsn_s2_w1p0_shuffle_roll1_lowlr1e3_nowarmup_ir2rgb_yolo11n_e200_b64_img512_s0_${stamp}"
  project_dir="runs_public/dronevehicle_method_search/sub2k_seed0_fullval/dsn_shared_private/s2_shuffled_controls/w1p0_roll1"
  log_root="logs/dronevehicle_method_search/sub2k_seed0_fullval/dsn_shared_private/s2_shuffled_controls/w1p0_roll1"
  ...
  echo "[$(date '+%F %T')] launching DSN shuffled control gpu=${gpu} run=${run_tag}"
  nohup env \
    PYTHON_BIN=/root/shared-nvme/venvs/ladd312/bin/python \
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    MODEL="$STUDENT" SAR_BASELINE="$STUDENT" RGB_TEACHER="$TEACHER" \
    INIT_TYPE=dsn_s2_w1p0_shuffle_roll1_dronevehicle \
    PAPER_RUN=0 PAPER_PROTOCOL_ID=dronevehicle_sub2k_lowlr_nowarmup_dsn_s2_w1p0_shuffle_roll1 \
    ...
    DSN_PROJECTOR_WEIGHTS="$DSN_CKPT" DSN_KD_WEIGHT=1.0 DSN_STUDENT_PROJECTOR=rgb DSN_TEACHER_PROJECTOR=peer TEACHER_BATCH_ROLL=1 \
    LADD_KD_DECAY_MODE=none LADD_KD_DECAY_START_EPOCH=-1 LADD_KD_DECAY_END_EPOCH=-1 LADD_KD_FINAL_MULT=1.0 \
  ...
  local pid="$!"
  echo "$pid" > "$pid_path"
  echo "[$(date '+%F %T')] DSN shuffled control pid=${pid} outer=${outer_log}"
}
...
echo "[$(date '+%F %T')] DSN shuffled-control queue ready refine_min_rows=${REFINE_MIN_ROWS} free_gpu_threshold=${FREE_GPU_THRESHOLD_MB}"
while ! refine_ready; do
  sleep "$POLL_SECONDS"
done

gpu=$(wait_for_free_gpu)
launch_shuffle "$gpu"
echo "[$(date '+%F %T')] DSN shuffled-control queue finished launching"
