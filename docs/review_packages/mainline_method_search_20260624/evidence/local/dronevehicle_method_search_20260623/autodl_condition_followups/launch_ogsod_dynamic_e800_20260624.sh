...
project="runs_public/dronevehicle_method_search/ogsod_nomosaic_autodl_condition/yolo_dynamic"
log_root="${ROOT_LOG}/ogsod_yolo_dynamic_gpu${GPU_OGSOD}"
master_log="${log_root}/master.log"
mkdir -p "$project" "$log_root"

if pgrep -af "train_ladd_hbb.py.*${project}.*--epochs 800" >/dev/null 2>&1; then
  echo "[$(date '+%F %T')] OGSOD e800 yolo_dynamic already appears to be running under ${project}; skip." | tee -a "$master_log"
  exit 0
fi

guard_gpu "$GPU_OGSOD" 9000 "OGSOD yolo_dynamic"

stamp=$(date +%Y%m%d_%H%M%S)
tag="ogsod_nomix_yolo_dynamic_existingcache_yolo11n_e800_b64_img256_s0_${stamp}"
run_name="${tag}_b"
...
chmod +x "$cmd_path"
{
  echo "[$(date '+%F %T')] launch OGSOD yolo_dynamic gpu=${GPU_OGSOD}"
  echo "tag=${tag}"
...
echo "$!" > "$pid_path"
echo "[$(date '+%F %T')] launched OGSOD yolo_dynamic pid=$(cat "$pid_path")" | tee -a "$master_log"
