...
launch_variant() {
  ...
  project_dir="runs_public/dronevehicle_method_search/sub2k_seed0_fullval/teacher_conf_gated_kd/${suffix}"
  log_root="logs/dronevehicle_method_search/sub2k_seed0_fullval/teacher_conf_gated_kd/${suffix}"
  ...
  echo "[$(date '+%F %T')] launching ${suffix} gpu=${gpu} run=${run_tag}"
  nohup env \
    PYTHON_BIN=/root/shared-nvme/venvs/ladd312/bin/python \
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
    MODEL="$STUDENT" SAR_BASELINE="$STUDENT" RGB_TEACHER="$TEACHER" \
    INIT_TYPE="teacher_conf_gated_kd_${suffix}_dronevehicle" \
    PAPER_RUN=0 PAPER_PROTOCOL_ID="dronevehicle_sub2k_lowlr_nowarmup_teacher_conf_gated_kd_${suffix}" \
    ...
    B_DETECTOR_SOURCE="$STUDENT" B_DECOMP_SOURCE="$DECOMP" B_LOAD_STUDENT_SPLIT=0 B_LOAD_STUDENT_REACHABILITY=0 \
    COMPARISON_KD_PROFILE=none PROFILE_KD_WEIGHT=0.0 PROFILE_KD_REPLACE_BASE=0 \
    STUDENT_BRANCH_MODE=raw STUDENT_DETECT_MODE=raw TEACHER_FEATURE_MODE=decomposed UNLEARNABLE_HIDDEN_RATIO=1.0 \
...
echo "[$(date '+%F %T')] teacher-conf/late-decay queue ready threshold=${THRESHOLD_MB} min_rows=${MIN_ROWS} require_cmdistill=${REQUIRE_CMDISTILL}"
while ! primary_ready; do
  sleep "$POLL_SECONDS"
done

gpu=$(wait_for_gpu)
launch_variant "$gpu" teacher_conf_gate teacher_task_conf none -1 -1 1.0
sleep 180

gpu=$(wait_for_gpu)
launch_variant "$gpu" rawkd_late_decay none linear 60 160 0.0

echo "[$(date '+%F %T')] teacher-conf/late-decay queue finished launching"
