...
- LD 检验只蒸馏定位输出分布是否足够。
- CCLKD-style 检验 category-constrained cross-modal contrastive KD。
...
不能进入修正版主表。CrossKD 与 CoLD 已统一归档。当前应先对 FGD/LD/CCLKD-style 做短 smoke，
确认 loss 非零、无 NaN/OOM、参数和增强一致，再启动正式实验。HalluciDet-style
现有运行可继续作为候选，但仍需跑满后判断。
