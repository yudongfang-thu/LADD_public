...
本目录保存 LADD 主方法重新定义之前的历史实验记录。归档不是删除证据，而是把旧口径从当前主线入口移开，避免 A1-A2-B、旧 mosaic 或调试型 loss 结果污染 `LADD Probe-A / clean_a1b_dynprobe` 主表。

## 当前有效口径

当前 LADD 主线只认：

- 方法：`LADD Probe-A / LADD-clean A1B (dynamic_probe)`
- 阶段：`A1 -> B`，不经过 A2
- 主线 tag：`clean_a1b_dynprobe`
- 当前证据入口：`docs/experiments/ladd_mosaic100_mainline_curves_20260618/`
- 方法定义：`docs/ladd_clean_a1b_method_definition.md`
- 训练规范：`docs/experiments/LADD_MAINLINE_STANDARD_CN.md`

`clean_a1b` static 和 `clean_a1b_dyn` dynamic 仍保留为消融实验；它们不是主线方法，但可以用于解释 Probe-A 中 teacher dynamic 与 frozen student probe 的贡献。
...
| `legacy_progress_snapshots/` | 旧 active run 快照、旧动态/静态 loss 临时分析、no-mosaic Probe-A 快照 | 只作运行过程追溯 |
...
5. 当前主表候选必须来自 `clean_a1b_dynprobe`，并满足同容量、同 seed、同协议 baseline 对齐。

## 仍在当前入口保留的 clean 结果

`docs/experiments/ladd_mosaic100_mainline_curves_20260618/` 不是旧 mosaic 归档包。它保存当前 clean A1B 口径下的 mosaic100 主线曲线，其中：

- `clean_a1b_dynprobe`：主线；
- `clean_a1b` static：消融；
