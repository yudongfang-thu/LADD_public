...
```bash
ssh ydf@10.103.12.90 # 注意走 SOCKS5 proxy
...
1. **找一个空闲 GPU**(比如 600EP baseline 跑完后 GPU 1/3 可能空出来):
   ```bash
   ssh ydf@10.103.12.90 'nvidia-smi --query-gpu=index,memory.used,utilization.gpu --format=csv'
   ```

2. **launch 修复版**:
   ```bash
   ssh ydf@10.103.12.90 'cd /mnt/dataY/ydf/projects/LADD_ref && \
