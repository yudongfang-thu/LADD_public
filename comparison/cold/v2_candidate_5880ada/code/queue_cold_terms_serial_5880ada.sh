...
QUEUE_LOG="${QUEUE_LOG:-$LOG_DIR/cold_terms_serial_e${EPOCHS}_b${BATCH_SIZE}_${RUN_TAG}_5880ada_queue.log}"
mkdir -p "$LOG_DIR"
export PYTHONPATH="/home/xmu/djd/ladd/.deps/python${PYTHONPATH:+:$PYTHONPATH}"

run_one() {
  local term="$1"
  local run_name="cold_v5p0_terms_${term}_t20_l1_topk1000_noiwm_e${EPOCHS}_b${BATCH_SIZE}_${RUN_TAG}_5880ada"
