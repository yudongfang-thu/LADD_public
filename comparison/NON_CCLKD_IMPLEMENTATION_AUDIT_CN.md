...
2026-06-10 代码已按本文边界修正：FGD-style 增加 fg/bg feature loss 与
student/teacher attention mask loss，LD 增加 teacher-quality VLR-style candidate
LD，HalluciDet portable baseline profile 改名为 `hallucidet_style`。

| 方法 | 当前代码判断 | 写作口径 |
|---|---|---|
| FGD-style | fg/bg feature + attention mask；GT-box mask 默认启用；legacy batch relation 默认关闭 | `FGD-style / FGD-YOLO adaptation` |
| LD | raw DFL foreground/main KL + teacher-quality VLR-style candidate KL | `LD adapted to YOLO11 DFL logits` |
| HalluciDet-style | profile 名称为 `hallucidet_style`；使用 RGB privileged teacher 做 feature/response/margin alignment | `HalluciDet-style, no explicit hallucination module` |
...
- 计算 student/teacher attention mask loss；
- feature loss 拆成 fg_loss 和 bg_loss。

需要保留的边界：

- 默认 `fgd_mask_mode=gt_box` 已使用 GT box 面积归一化 mask；`assigner`
  仅作为 fallback；
- 官方 Global KD 是带 `conv_mask_*` 和 `channel_add_conv_*` 的 GCNet-style
  context 模块；本地旧 batch relation 已改为 legacy opt-in，默认
  `fgd_lambda=0.0` 不启用；
...
- foreground/main LD 使用 assigned target quality 和 teacher assigned-class
  confidence 加权；
- VLR-style candidate LD 在非 foreground anchors 上用 teacher confidence 与
  teacher-box/GT IoU 构造权重；
...
- YOLO11 TaskAlignedAssigner 不暴露官方 ATSS `get_vlr_region()` API；当前 VLR
  是 teacher-quality candidate adaptation；
- 本地没有完整 classification KD / IM recipe；
...
旧 `hallucidet` 名称不再被 CLI/launcher 接受。
...
当前两个 non-CCLKD launcher 都限制 method 为 `fgd|ld|hallucidet_style`，不会误启动
CCLKD：
...
1. 对 `fgd`、`ld`、`hallucidet_style` 各跑 1-2 epoch smoke。
...
5. 修复前 FGD/LD/HalluciDet 旧结果不得与当前实现结果混合统计；旧
   hallucidet 只能标为 `hallucidet_style_old`。
