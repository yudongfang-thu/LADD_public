...
DATA = ROOT / "data"
PROBE = DATA / "alignment_probe_20260618"
OUT = ROOT / "plots"
...
RUNS = [
    ("native_noalign_nokd", "default aug no KD", "native_noalign_nokd_results.csv"),
    ("aligned_nocolor_nokd", "aligned no color no KD", "aligned_nocolor_nokd_results.csv"),
    ("aligned_color_nokd", "aligned color no KD", "aligned_color_nokd_results.csv"),
]
...
def load_runs() -> dict[str, tuple[str, list[dict[str, float]]]]:
    ...
    for key, label, filename in RUNS:
        path = PROBE / filename
        if path.exists():
            loaded[key] = (label, read_rows(path))
    return loaded
...
def plot(runs: dict[str, tuple[str, list[dict[str, float]]]]) -> None:
    ...
    for idx, (key, (label, rows)) in enumerate(runs.items()):
        ...
        ax.plot(epochs, m50, color=color, linewidth=1.4, alpha=0.65, label=label)
        ax.plot(epochs, moving_average(m50), color=color, linewidth=2.2, alpha=0.32)
        b = best(rows, "metrics/mAP_0.5")
        ax.scatter([b["epoch"]], [b["metrics/mAP_0.5"]], color=color, s=28, zorder=4)
        ax.annotate(
    ...
    ax.axhline(CMDISTILL_TABLE_III_NO_KD, color="0.15", linestyle=":", linewidth=1.2, label="paper Table III no KD")
    ax.set_xlabel("Epoch")
    ...
    for idx, (key, (label, rows)) in enumerate(runs.items()):
        color = colors[idx % len(colors)]
        ax.plot(get(rows, "epoch"), get(rows, "metrics/mAP_0.5:0.95"), color=color, linewidth=1.4, label=label)
    ax.set_xlabel("Epoch")
    ...
    for idx, (key, (label, rows)) in enumerate(runs.items()):
        color = colors[idx % len(colors)]
        ax.plot(get(rows, "epoch"), det_loss(rows, "val"), color=color, linewidth=1.3, label=f"{label} val")
    ax.set_xlabel("Epoch")
    ...
    fig.savefig(OUT / "cmdistill_alignment_probe_20260618_trend.pdf")
    fig.savefig(OUT / "cmdistill_alignment_probe_20260618_trend.png")
    plt.close(fig)


def write_summary(runs: dict[str, tuple[str, list[dict[str, float]]]]) -> None:
    lines = [
        "| run | epochs logged | last epoch | latest mAP50 | best mAP50 | best epoch | latest mAP50-95 | best mAP50-95 | best mAP50-95 epoch |",
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for _, (label, rows) in runs.items():
        ...
        lines.append(
            f"| {label} | {len(rows)} | {int(rows[-1]['epoch'])} | "
    ...
    lines.append("")
    lines.append(f"References: our RGB best={RGB_BASELINE_BEST:.4f}; CMDistill Table III no-KD={CMDISTILL_TABLE_III_NO_KD:.3f}.")
    OUT.mkdir(parents=True, exist_ok=True)
    (OUT / "cmdistill_alignment_probe_20260618_summary.md").write_text("\n".join(lines) + "\n")
...
def main() -> None:
    ...
    runs = load_runs()
    if not runs:
        raise FileNotFoundError(f"No probe CSVs found in {PROBE}")
    plot(runs)
    write_summary(runs)
